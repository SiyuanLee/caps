...
import dqn_evaluation
from dqn_utils import *
from option import PrimitiveOption, Source


def transfer_model(img_in, num_actions, num_options, scope, reuse=False, omega_stop=False, termination_stop=False, no_share_para=False, beta_no_bias=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("termination_prob"):
            ...
            if beta_no_bias:
                term_prob = layers.fully_connected(term_in, num_outputs=num_options, activation_fn=tf.sigmoid, biases_initializer=None)
            else:
...
def transfer_learn(env,
    ...
    lr_schedule_omega = PiecewiseSchedule([], outside_value=2.5e-4)
    ...
    optimizer = dqn_evaluation.OptimizerSpec(
        constructor=tf.train.AdamOptimizer,
        kwargs=dict(epsilon=1e-4),
        lr_schedule=lr_schedule
    )

    optimizer_omega = dqn_evaluation.OptimizerSpec(
        constructor=tf.train.AdamOptimizer,
    ...
    optimizer_term = dqn_evaluation.OptimizerSpec(
