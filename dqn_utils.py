...
def pick_option(opa_batch):
    """Helper function. random sample non zero index from opa_batch"""
    batch_size, num_option = opa_batch.shape
    ret = np.empty([batch_size], dtype=opa_batch.dtype)
    for i in range(batch_size):
        ret[i] = random.choice([j for j in range(num_option) if opa_batch[i][j] > 0])
    return ret
...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        if self.opa is None:
            self.opa = np.empty([self.size], dtype=np.bool)
        ...
        opa_batch      = self.opa[idxes]

        return obs_batch, act_batch, rew_batch, next_obs_batch, done_mask, opa_batch


    def sample(self, batch_size, uniform_opa=False):
        ...
        done_mask: np.array
            Array of shape (batch_size,) and dtype np.float32
        opa_batch: np.array
            Array of shape (batch_size, num_options) and dtype np.int32
        """
        assert self.can_sample(batch_size)
        if uniform_opa:
            idxes = self.sample_n_unique_opa_uniform(lambda: random.randint(0, self.num_in_buffer - 2), batch_size)
        else:
            idxes = sample_n_unique(lambda: random.randint(0, self.num_in_buffer - 2), batch_size)
        return self._encode_sample(idxes)

    def sample_n_unique_opa_uniform(self, sampling_f, n):
        """roughly the same as sample_n_unique except that
        the prob of sampling the index i is proportional to
        num of non zero entries of opa
        """
        res = []
        while len(res) < n:
            candidate = sampling_f()
            if candidate not in res:
                samp_r = np.sum([
                    1.0 if self.opa[candidate][i] > 0 else 0.0 for i in range(self.opa.shape[1])
                ]) / self.opa.shape[1]
                if random.random() <= samp_r:
                    res.append(candidate)
        return res
    ...
    def store_effect(self, idx, action, reward, done, opa):
        ...
        opa: int array
            Options whose actions are the same as the action picked. 2 for option running, 1 for others
        """
        if self.opa is None:
            self.opa = np.empty([self.size] + list(opa.shape), dtype=np.int32)
...
class NoOpWrapperMK(gym.Wrapper):
    ...
    #def _step(self, action):
    #    action = action if action != 5 else 3  # change up if given noop
