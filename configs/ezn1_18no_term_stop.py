...
pkl_file = '/home/beeperman/Project/ple-monsterkong/examples/dqn_transfer_option/configs/eval_obs_array_batch_ezn_strange.pkl'
...
# I think need to change, the simpler, the better
learning_rate_term = PiecewiseSchedule([
    (0, 2e-5 * lr_multiplier),
    (num_iter / 20, 2.5e-4 * lr_multiplier),
    (num_iter / 5, 1e-3 * lr_multiplier),
...
transfer_config = {
    ...
    # change
    'four_to_two': True,  # default false frame_history_len must be 4! what's meaning?
...
dqn_config = {
    ...
    'room_q_interval': 1e5,  # q_vals will be evaluated every room_q_interval steps, not use in dqn
    'epoch_size': 5e4,  # you decide any way, for all logs
