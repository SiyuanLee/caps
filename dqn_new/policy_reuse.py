...
def atari_model(img_in, num_actions, scope, reuse=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("feature_value"):
            # out = tf.layers.dense(out,512)
            out = layers.fully_connected(out, num_outputs=512, activation_fn=None)
            # out = layers.fully_connected(out, num_outputs=num_actions, activation_fn=None)
        with tf.variable_scope("q_omega"):
            q_omega = layers.fully_connected(out, num_outputs=num_actions + 3, activation_fn=tf.nn.softmax)
            # q_omega = layers.softmax(q_omega_linear)
        with tf.variable_scope("beta"):
            # 3 is the number of source policies
            beta = layers.fully_connected(out, num_outputs=num_actions + 3, activation_fn=tf.nn.sigmoid)
        with tf.variable_scope("q"):
            q_1 = tf.nn.relu(out)
            q_out = layers.fully_connected(q_1, num_outputs=num_actions, activation_fn=None)


        return q_omega, beta, q_out
...
def main():
    ...
    if map_config['dqn_config'] and map_config['dqn_config'].has_key('seed'):
        if callable(map_config['dqn_config']['seed']):
            seed = map_config['dqn_config']['seed']()
        else:
            seed = map_config['dqn_config']['seed']
    env = wrap_env(env, seed)
    env_test = wrap_env(env_test, seed)
    session = get_session()
    atari_learn(env, env_test, session, num_timesteps=num_timesteps, dqn_config=map_config['dqn_config'])
