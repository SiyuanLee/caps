...
def learn(env,
    ...
    total_error = tf.reduce_mean(tf.losses.mean_squared_error(rew_t_ph + gamma * q_val_next, q_val_current))
    ...
    for t in itertools.count():
        ...
        if random.random() > epsilon and model_initialized:
            ...
            q_vals = session.run(q_target, {obs_tp1_ph: input_batch[None, :]})
        ...
        if (t > learning_starts and
            ...
            if t % epoch_size == 0 and log_dir != None and model_initialized:
                ...
                if eval_obs_array:
                    ...
                    for eval_obs in eval_obs_array:
                        q_vals_eval_obs = []
                        for ob in eval_obs:
                            q_vals_eval_obs.append(session.run(q_max, {obs_t_ph: ob})[0])
                        q_vals_eval_obs_array.append(q_vals_eval_obs)
                    with open(log_dir + 'pkl/' + str(t) + "_q.pkl", 'wb') as f:
            ...
            if t % epoch_size == 0 and log_dir != None and model_initialized:
                ...
                plt.figure(1)
                Average_costs.append(np.mean(Costs[int(-epoch_size):]))
                ...
                with open(log_dir + 'average_q.pkl', 'wb') as output:
                    pickle.dump(average_Qs, output)