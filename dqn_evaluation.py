...
def learn(env,
    ...
    for t in itertools.count():
        ...
        if j < 5:
            ...
            print "option running", option_running
            # print "action", key
            # if key == 's':
            #   action = 1
            # elif key == 'a':
            ...
            # print "Q", Q_Current1
            # print "best action", np.argmax(Q_Current1)
        ...
        print np.array(list(new_obs)).shape
        im = Image.fromarray(np.array(list(new_obs)).astype(np.uint8)[:,:,0])
        # im.show()
        if done:
            # print "R:",r
            new_obs = env.reset()
        ...
        if not model_initialized:
            ...
            checkpoint = tf.train.get_checkpoint_state(
                "/home/lsy/Desktop/same_option/dqn")
