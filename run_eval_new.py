...
def transfer_learn(env,
    ...
    optimizer = DQN_eval.OptimizerSpec(
        constructor=tf.train.AdamOptimizer,
        kwargs=dict(epsilon=1e-4),
        lr_schedule=lr_schedule
    )

    optimizer_omega = DQN_eval.OptimizerSpec(
    ...
    optimizer_term = DQN_eval.OptimizerSpec(
        constructor=tf.train.GradientDescentOptimizer,
        #kwargs=dict(epsilon=1e-4), not for SGD
        kwargs=dict(),
...
def main():
    ...
    session = get_session()
    transfer_learn(env, env_test, session, num_timesteps=num_timesteps, dqn_config=dqn_config)
