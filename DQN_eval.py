...
def learn(env,
    ...
    u_next_raw = (1 - term_next_current) * q_omega_target + term_next_current * max_q_omega_next_targ
    u_next = u_next_raw * (1 - done_mask_ph)

    # q_omega_value error
    total_error_q_omega = tf.reduce_mean(tf.reduce_sum(
        tf.one_hot(opa_t_ph, num_options) *
        tf.losses.mean_squared_error(rew_t_ph + gamma * u_next, q_omega_current),
        axis=-1
    ))
    ...
    for t in itertools.count():
        ...
        _, _, loss_omega, loss_term = session.run(
            [train_fn_omega, train_fn_term, total_error_q_omega, total_error_term], feed_dict={
                obs_t_ph: recent_obs[None, ..., -input_history_len:],
        ...
        [session.run(train_fn_omega, feed_dict={
            obs_t_ph: recent_obs[None, ..., -input_history_len:],
            opa_t_ph: [i],
            rew_t_ph: [r],
            obs_tp1_ph: next_recent_obs[None, ..., -input_history_len:],
            done_mask_ph: [1.0 if done == True else 0.0],
            learning_rate_omega: optimizer_spec_omega.lr_schedule.value(t / learning_freq),
        }) for i in range(num_options) if i != option_running and options[i].act(recent_obs) == action]
        ...
        if (t > learning_starts and
            ...
            if not q_model_initialized:
                ...
                checkpoint = tf.train.get_checkpoint_state(
                    "/home/lsy/PycharmProjects/ple-monstrerkong/examples/dqn_transfer_option/logs/12_16sep_12_16_17_10:05:16/dqn")
            ...
            _, cost = session.run([train_fn, total_error_q], feed_dict={
        ...
        if t % LOG_EVERY_N_STEPS == 0 and q_model_initialized:
            ...
            if (t - learning_starts) % epoch_size == 0 and log_dir != None:
                ...
                with open(log_dir + 'test_rewards.pkl', 'wb') as output:
                    pickle.dump(test_rewards, output)
                with open(log_dir + 'none_discount.pkl', 'wb') as output:
                    pickle.dump(none_discount, output)
                with open(log_dir + 'short_average.pkl', 'wb') as output:
                    pickle.dump(short_average, output)
                with open(log_dir + 'short_average_none.pkl', 'wb') as output:
